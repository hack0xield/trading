...
Every event, including each order before it is sent, goes to the console,
to `runs-live/<strategy>_<symbol>_<magic>/events.jsonl`, and to any
`--webhook`. Ctrl-C in the wrapper stops it, as does creating the stop file.

Credentials are read from `mt5-mcp-server/config.json`.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    output = parser.add_argument_group("events")
    output.add_argument("--events",
                        help="events JSONL (default runs-live/<strategy>_<symbol>_<magic>/)")
    output.add_argument("--webhook", action="append", default=[], metavar="URL",
                        help="POST every event as JSON here; repeatable")
...
def main(argv: list[str] | None = None) -> int:
    ...
    strategy = get_strategy(config["strategy"])(config.get("params"))
    if getattr(strategy.p, "place_orders", True) is False:
        print("  ! place_orders is false: the strategy will trade nothing")
    magic = args.magic if args.magic is not None else default_magic(strategy.name, symbol)
    session = LIVE_DIR / f"{strategy.name}_{symbol}_{magic}"
    events = Path(args.events) if args.events else session / "events.jsonl"
    ...
    try:
        ...
        raise SystemExit(
            ...
            "    scripts/run-live.sh " + " ".join(sys.argv[1:])
        )
    ...
    try:
        ...
        notify = Notifier(strategy.name, symbol)
        if not args.quiet:
            notify.add(ConsoleListener())
        notify.add(JsonlListener(events))
        for url in args.webhook:
            notify.add(WebhookListener(url))

        print(
            f"{strategy.describe()} on {symbol} {timeframe}\n"
            f"  account  {account.login} @ {account.server} ({trade_mode_name(mt5, account)})\n"
