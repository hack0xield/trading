...
def spec_from_mt5(info) -> Instrument:
    ...
    values, while this is what your broker will actually charge you.
    """
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if info is None:
            raise SystemExit(f"Unknown symbol {args.symbol}: {mt5.last_error()}")

        if args.dump_spec:
            path = save_instrument(spec_from_mt5(info))
