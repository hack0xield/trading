...
- **Account guard**: nothing is sent unless the terminal is still on the login
  ...
  rejection.
- **Broker answers**: done, refused, "not now" (kept and resent after a wait
  doubling from `--retry-seconds` to `--retry-max-seconds`; market closed at
  the daily break is routine and silent), or possibly done (looked for at the
  broker before it is counted or sent again). Only a refusal, or an entry
  unsent when its bar ends, is rejected.
